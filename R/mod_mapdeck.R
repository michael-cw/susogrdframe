...
mapModuleSvr <- function(id,
  ...
  moduleServer(id, function(input, output, session){
    ...
    output$baseMap1<-mapdeck::renderMapdeck({
      m<-mapdeck(token = key, location = startlocation, zoom = startzoom,
    ...
    # return(
    #   list(
    #     clickScatter = clickScatter()
    #   )
    # )
    ##################################################################################################
    ### Update Style: ATTENTION NO FIXED YET. CHECK BACK!!!
    ##############################################################
    # observeEvent(input$maptype, {
    #   req(input$maptype)
    #   mapdeck_update(map_id = session$ns("baseMap1")) %>%
    #     update_style (style = mapdeck_style(input$maptype))
    # })
...
# mod_mapdeck_ui("mapdeck_1")

## To be copied in the server
# mod_mapdeck_server("mapdeck_1")
