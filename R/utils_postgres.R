...
shapeLoad2<-function(path=NULL, lay=NULL, sp.Library="sf"){
  ...
  if (sp.Library=="sp") {
    ...
    stop("SP not supported any longer. Please choose sf and transform in case you require sp format.")
    # ###############################################################################################
    # #   Upload the shape SP
    # ###############################################################################################
    # harare_landuse3<-rgdal::readOGR(dsn = path, layer = lay)
    # outlist[[1]]<-rgdal::ogrInfo(dsn = path, layer = lay)
...
###############################################################################################
shapeLoad2_cleanToDB<-function(SHP = NULL, shpName = NULL, writeToDB = T) {
  crsOld<-st_crs(SHP)
  ##  ii. Remove empty geometries first
  if(sum(st_is_empty(SHP))>0) {
    SHP<-SHP[!st_is_empty(SHP),]
  ...
  if(writeToDB){
    ...
    ## b. write & return list
    dbTableList<-writeSFtoDB(object = SHP, fn = tableName)
  }
...
readSHPfromDB<-function(dbname = NULL, host = NULL,
                        user = NULL, password = NULL,
                        fn = NULL, listTables = F, inShinyApp = T) {
...
################################################################################################
writeRAStoDB<-function(object = NULL, dbname = "spatsampdata", host = "localhost",
                       user = "shiny", password = "nitro12345",
                       fn = NULL, listTables = F) {
...
readRASfromDB<-function(dbname = "spatsampdata", host = "localhost",
                        user = "shiny", password = "nitro12345",
                        fn = NULL, listTables = F) {
